...
class DoublePendulumEnv(gym.Env):
    ...
    def __init__(self, render_sim=False, n_steps=1000, m1 = 1.0, m2 = 1.0, L1 = 160, L2 = 160, mode = 'balance'):
        self.m1 = m1
        self.m2 = m2
    ...
    def step(self, action):
        ...
        #Current step
        x = int(self.current_time_step) 

        #For staying balanced
        if np.abs(obs[0]) <= 1.0/18 and np.abs(obs[1]) <= 1.0/18 and np.abs(obs[2]) <= 2.0/18 and np.abs(obs[3]) <= 2.0/18:
            fstand[x]=True
        else:
            fstand[x]=False
'''
        #Post Standing position
        if np.abs(obs[0]) <= 2.0/18 and np.abs(obs[1]) <= 2.0/18 and np.abs(obs[2]) <= 1.2/18 and np.abs(obs[3]) <= 1.2/18:
            pstand=True
        else:
            pstand=False
        
        #Pre-final position
        if np.abs(obs[0]) <= 0.8/18 and np.abs(obs[1]) <= 0.8/18 and np.abs(obs[2]) <= 0.45/18 and np.abs(obs[3]) <= 0.45/18:
            fullstand=True
        else:
            fullstand=False

        #Final position
        if np.abs(obs[0]) <= 0.1/18 and np.abs(obs[1]) <= 0.1/18 and np.abs(obs[2]) <= 0.15/18 and np.abs(obs[3]) <= 0.15/18:
            fstandf=True
        else:
            fstandf=False

        #standing flag
        if np.abs(obs[1]) <= 1.8/18:
            stand = True
        else:
            stand = False
        ...
        if self.mode == 'balance':
            ...
            Dis =10-J+((7 - ((np.sin(np.abs(obs[0]*np.pi))*3.0)+(np.sin(np.abs(obs[1]*np.pi))*3.0)))**2) + (((np.cos(np.abs(obs[0]*np.pi))*3.0)+(np.cos(np.abs(obs[1]*np.pi))*3.0))**2)
            
            if np.abs(obs[1]) <= 2.0/18 and np.abs(obs[0]) <= 9/18:
                reward=Dis
                if abs(obs[2]) >= 13.0/18 or abs(obs[3]) >= 9.0/18:
                    reward = -(0.1* Dis)
            else:
                reward=-30
            ...
            if pstand==True and np.abs(obs[0]) <= 2.0/18 and np.abs(obs[1]) <= 2.0/18 and np.abs(obs[2]) <= 1.2/18 and np.abs(obs[3]) <= 1.2/18:
                reward = (3*Dis)-(J)
                print("!! Trying to stand !!&&")
            if fullstand==True:
                reward = (3.5 *Dis)-J

            if fullstand==True and np.abs(obs[0]) <= 0.8/18 and np.abs(obs[1]) <= 0.8/18 and np.abs(obs[2]) <= 0.45/18 and np.abs(obs[3]) <= 0.45/18:
                reward = (4*Dis)-(J)
                print("!! Pre-Final position Standing !!&&")

            if fstandf==True:
                reward = (4.5*Dis)-(J)

            if fstandf==True and np.abs(obs[0]) <= 0.1/18 and np.abs(obs[1]) <= 0.1/18 and np.abs(obs[2]) <= 0.15/18 and np.abs(obs[3]) <= 0.15/18:
                reward = (8*Dis)-(J)
                print("&&$$ Final position Standing $$&&")                

                #print("Swinging-up")

            #Penalty for loss of balance
            if np.abs(obs[1]) > 1.8/18 and stand == True:
                reward = - (0.7 * Dis)
                print("Falling","dtheata1: ",obs[2],"  ","dtheata2: ",obs[3],"  ")

            #Penalty for falling from standing position
            if pstand==True and (np.abs(obs[0]) > 2.0/18 or np.abs(obs[1]) > 2.0/18):
                reward = - (0.7*Dis)
            #Penalty for falling 9 times in the same episode    
